...
export function summarize(orders) {
  ...
  for (const o of orders) {
    ...
    const s = bySeller[o.seller || '(no name)'] ??= { orders: 0, total: 0 };
...
/** The report as HTML. `deletable` adds a Delete button to each order (the Scout's own device only). */
export function renderReport(orders, { deletable = false, showSellers = false } = {}) {
  ...
  return `
    ...
    <div class="card"><table>
      <tr><th>Scout</th><th class="num">Orders</th><th class="num">Collected</th></tr>
      ${rows(Object.entries(s.bySeller).sort((a, b) => b[1].total - a[1].total), ([name, v]) => `<td>${esc(name)}</td><td class="num">${v.orders}</td><td class="num">${fmt(v.total)}</td>`)}
    </table></div>` : ''}
    <h2>By product</h2>
    <div class="card"><table>
    ...
    ${s.checks.length ? `<h2>Checks</h2>
    <div class="card"><table>
      <tr><th>Check #</th><th>From</th><th class="num">Amount</th></tr>
      ${rows(s.checks, o => `<td>${esc(o.payment.checkNumber) || '<span class="hint">none</span>'}</td><td>${esc(o.customer.name)}</td><td class="num">${fmt(o.total)}</td>`)}
    </table></div>` : ''}
